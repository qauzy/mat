...
int tc_redir_egress_func(struct __sk_buff *skb) {
    ...
    __u32 key = 0, *redir_ip, *redir_port; // *mat_mark

//    mat_mark = bpf_map_lookup_elem(&redir_params_map, &key);
//    if (mat_mark && *mat_mark != 0 && *mat_mark == skb->mark)
//        return TC_ACT_OK;
